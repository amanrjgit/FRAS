...
def recognize_faces_from_camera_no_cascade(model, index_to_name):
    ...
    # Set the font for displaying the recognized name
    ...
    while True:
        ...
        # Detect faces in the frame using the model
        while counter < 50:
            for y in range(0, gray.shape[0], img_size[0]):
                for x in range(0, gray.shape[1], img_size[1]):
                    # Extract the face from the frame
                    face = gray[y:y+img_size[0], x:x+img_size[1]]
                    ...
                    # Add the face to the list of faces to recognize
                    faces.append(face)
                    rects.append((x, y, img_size[1], img_size[0]))
                    counter += 1

        # Make a prediction for each face using the model
        if len(faces) > 0:
            faces = np.array(faces)
            predictions = model.predict(faces)
            # Display the recognized name for each face
            name_list = []
            for i in range(len(predictions)):
                ...
                if idx in index_to_name:
                    temp_name = index_to_name[idx]
                else:
                    temp_name = 'Unknown'
                name_list.append(temp_name)
                count = Counter(name_list)
                name = max(count, key=lambda k: count[k])
                faces = []
...
recognize_faces_from_camera_no_cascade(model,index_to_name=person_dict)
